...
### Recent work (on `main` / `origin/main`)

Last 4 merged commits were wss/hosted-server support (`git log origin/main --oneline`):

- `Add wss support for Pages + hosted server` — `packages/client/src/net.ts:94` now resolves `ws://` vs `wss://` (supports `?host=wss://…`, `?secure`/`?wss`/`?tls` params, same-host port handling), plus `packages/server/src/index.ts:1` http health endpoints (`/health`, `/healthz`) and `fly.toml:1`/`Dockerfile:1` for Fly deployment.
- Follow-up review fixes — health slash, LAN log, Dockerfile prune, same-host https wss regression, Docker client dep pruning.

Active branches (as of last fetch): `feat/m2-combat`, `feat/m3-polish`, `fix/wss-review-fixes`, `docs/mark-m2-done` (all merged to `main`). Working tree clean.

## Architecture — read before changing netcode/physics

- **Authoritative server** owns all sim: `packages/server/src/room.ts:70` (`Room`) runs the fixed-step loop at `TICK_RATE_HZ=60` (`packages/shared/src/constants.ts:14`), `SNAPSHOT_RATE_HZ=60`, `INTERP_DELAY_MS=50`. Clients send input bitmasks; server broadcasts snapshots.
...
- Fly server: `Dockerfile:1` copies only `shared`+`server` + `bun install --frozen-lockfile`, `fly.toml:1` (`app=jumpshot`, `primary_region=bom`, `internal_port=8080`, `auto_stop_machines=stop`).
